    ...
    <div class="pinkbox">
      ...
      <div class="signup nodisplay">
      	...
      	<form action="regiAf.do" method="post" autocomplete="off">
      	  <div class="id_check_button">
          	<input type="text" id="reg_id" name="userId" placeholder="아이디">
          	<button type="button" id="idChkBtn">중복 체크</button>
          </div>
          <p id="idcheck" style="display:none;"></p> 
          <input type="password" id="reg_pwd" name="password" placeholder="비밀번호">
          <input type="password" id="reg_pwd_2" placeholder="비밀번호 확인">
	  ...
	  <div class="signin">
	  	...
	  	<form action="loginAf.do" method="post" class="more-padding" autocomplete="off">
			<input type="text" id="sign_id" name="userId" placeholder="아이디">
			<input type="password" id="pwd" name="password" placeholder="패스워드">
         	<div class="checkbox">
...
$(function() {
	...
	$("#create_id").click(function(event){
		...
		// 빈 칸이 있으면 알림
	    if ($("#reg_id").val().trim() == "" || $("#reg_pwd").val().trim() == "" || $("#reg_pwd_2").val().trim() == "" 
	    		|| $("#name").val().trim() == "" || $("#age").val().trim() == "" || $("#address").val().trim() == ""
	    		|| $("#email").val().trim() == "") {
	      alert("빈 칸을 모두 채워주세요.");
	...
	$("#chk_save_id").click(function () {
		...
		if( $("#chk_save_id").is(":checked") == true ){
			...
			if( $("#sgin_id").val().trim() == "" ){
				...
				// cookie를 저장
				$.cookie("userId", $("#sign_id").val().trim(), { expires:7, path:'./' });	// expires 만료 날짜, ./ 현재 경로
			}
